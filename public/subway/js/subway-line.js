class Line {
      ...
      for (var q = 0; q <= line_length; q++) {
        ...
        for (var r = 1; r <= line_length; r++) {
          ...
          total_distance += Math.pow(
            Math.pow(st_prev.lat - st_next.lat, 2) +
              Math.pow(st_prev.lng - st_next.lng, 2),
            0.5
          );
        }
  ...
  generate_draw_map() {
    ...
    for (var i = 0; i < this.stations.length - 1; i++) {
      ...
      if (station && station.lines.length > 1) {
          ...
          if (relevant_line_id != this.id) {
            ...
            for (var dir = -1; dir < 2; dir += 2) {
              if (dir > 0) {
                var delta =
                  Math.min(
                    relevant_line.stations.length - 1,
                    start_index + SHARED_STRETCH_THRESHOLD
                  ) - start_index;
              } else {
                var delta =
                  start_index -
                  Math.max(0, start_index - SHARED_STRETCH_THRESHOLD);
              }
              ...
              for (
                var k = start_index;
                k >= start_index - delta && k <= start_index + delta;
                k += dir
              ) {
                station_buffer.push(relevant_line.stations[k]);

                var station_is_shared_between_lines = is_in_array(
                  this.id,
                  N_stations[relevant_line.stations[k]].lines
                );
                var station_is_close_enough_on_impacted_line =
                  Math.abs(
                    i - this.stations.indexOf(relevant_line.stations[k])
                  ) <= SHARED_STRETCH_THRESHOLD;
                var different_stations =
                  relevant_line.stations[k] != station_id;
                var more_stations_on_relevant_line =
                  Math.abs(k - start_index) >
                  this.stations.indexOf(relevant_line.stations[k]) - i;
                var is_next_station_on_line =
                  relevant_line.stations[k] == this.stations[i + 1];

                if (
                  station_is_shared_between_lines &&
                  station_is_close_enough_on_impacted_line &&
                  different_stations &&
                  more_stations_on_relevant_line &&
                  is_next_station_on_line
                ) {
    ...
    if (USE_CURVED_TRACKS) {
      if (this.draw_map.length > 1) {
        ...
        for (var i = 0; i < this.draw_map.length; i++) {
          coordinates.push({
            x: N_stations[this.draw_map[i]].marker.getLatLng().lng,
            y: N_stations[this.draw_map[i]].marker.getLatLng().lat
          });
        }
        ...
        for (i = 1; i < this.draw_map.length; i++) {
          var bezier_options = [
            'M',
            [
              N_stations[this.draw_map[i - 1]].marker.getLatLng().lat,
              N_stations[this.draw_map[i - 1]].marker.getLatLng().lng
            ]
          ];
          this.control_points[i - 1] = [
            [spline.controls[i - 1][1].y, spline.controls[i - 1][1].x],
            [spline.controls[i][0].y, spline.controls[i][0].x]
          ];
        }
      }
    }
  ...
  draw() {
    ...
    if (USE_CURVED_TRACKS) {
      if (this.draw_map.length > 1) {
        for (i = 1; i < this.draw_map.length; i++) {
          if (!N_stations[this.draw_map[i - 1]]) {
            continue;
          }
          var bezier_options = [
            'M',
          ...
          var common_tracks = sort_by_group(
            intersect(station_drawmaps, station_next.drawmaps())
          );
          var unique_groups = lines_to_groups(common_tracks).sort();

          var first_line = N_lines[common_tracks[0]];
          var parity =
            first_line.draw_map.indexOf(this.draw_map[i]) >
            first_line.draw_map.indexOf(this.draw_map[i - 1]);
          ...
          if (station_drawmaps.length == 1) {
            ...
            var control_points_to_average = [
              [this.control_points[i - 1][0], this.control_points[i - 1][1]]
            ];
            ...
            for (var j = 0; j < station_drawmaps.length; j++) {
              ...
              if (station_drawmaps[j] != this.id) {
                var station_position_in_line = N_lines[
                  station_drawmaps[j]
                ].draw_map.indexOf(station_prev.id);

                // If there's a "next" station to check...
                if (
                  station_position_in_line + 1 <
                  N_lines[station_drawmaps[j]].draw_map.length
                ) {
                  if (
                    is_in_array(
                      N_lines[station_drawmaps[j]].draw_map[
                        station_position_in_line + 1
                      ],
                    ...
                    var cp_to_push =
                      N_lines[station_drawmaps[j]].control_points[
                        station_position_in_line
                      ];
                    if (cp_to_push != null) {
          ...
          bezier_options.push([
            N_stations[this.draw_map[i]].marker.getLatLng().lat,
            N_stations[this.draw_map[i]].marker.getLatLng().lng
          ]);

          // For testing control points
          if (DEBUG_MODE) {
            debug_layer.addLayer(
              L.circle([control_point_1[0], control_point_1[1]], 10, {
                stroke: false,
                color: 'blue',
                fillOpacity: 1.0
              })
            ...
            debug_layer.addLayer(
              L.circle([control_point_2[0], control_point_2[1]], 10, {
                stroke: false,
                color: 'red',
                fillOpacity: 1.0
              })
        ...
        if (N_stations[this.draw_map[this.draw_map.length - 1]]) {
          // Adjust marker style for the station outside the loop.
          N_stations[
            this.draw_map[this.draw_map.length - 1]
          ].set_marker_style();
        }
      }
    } else {
      for (i = 1; i < this.draw_map.length; i++) {
        ...
        // Get the number of colors in the tracks between these stations.
        var common_tracks = sort_by_group(
          intersect(station_prev.drawmaps(), station_next.drawmaps())
        );
        var unique_groups = lines_to_groups(common_tracks).sort();
        ...
        if (
          lines_to_groups(station_prev.drawmaps()).length >=
            STATION_MARKER_LARGE_THRESHOLD ||
          station_prev.lines.length > 8
        ) {
          station_prev.marker.setRadius(MARKER_RADIUS_LARGE);
        }

        var track = L.polyline(
          [station_prev.marker.getLatLng(), station_next.marker.getLatLng()],
          curve_options
        );
  ...
  for (var i = 0; i < impacted_lines.length; i++) {
    ...
    for (var j = 0; j < impacted_line.stations.length - 1; j++) {
      if (!is_in_array(impacted_line.stations[j], impacted_line.draw_map)) {
        impacted_line.draw_map.splice(
          draw_map_index,
          0,
          impacted_line.stations[j]
        );
      }
        ...
        for (var k = 0; k < relevant_lines.length; k++) {
          ...
          if (relevant_line_id != impacted_line_id) {
            ...
            for (var l = start_index; l <= end_index; l++) {
              ...
              var station_is_shared_between_lines = is_in_array(
                impacted_line_id,
                N_stations[relevant_line.stations[l]].lines
              );
              var station_is_close_enough_on_impacted_line =
                Math.abs(
                  j -
                    impacted_line.stations.indexOf(relevant_line.stations[l].id)
                ) <= SHARED_STRETCH_THRESHOLD;
              ...
              var more_stations_on_relevant_line =
                ...
                impacted_line.stations.indexOf(relevant_line.stations[l].id) -
                  j;
              var is_next_station_on_line =
                relevant_line.stations[l].id ==
                impacted_line.stations[j + 1].id;
              ...
              if (
                ...
                if (impacted_line_id == 2) {
                  var end_of_stretch_station =
                    N_stations[relevant_line.stations[l]];
                  var breakpoint = 0;
                }
    ...
    impacted_line.draw_map.splice(
      draw_map_index,
      0,
      impacted_line.stations[impacted_line.stations.length - 1]
    );
  }
}
...
function generate_route_diagram(line) {
  ...
  for (var i = 0; i < line_stations.length; i++) {
    ...
    var station_entry =
      '<div class="route-diagram-entry"><div class="route-diagram-name">' +
      station.name +
      '</div><div class="route-diagram-transfers">';
    for (var j = 0; j < station.lines.length; j++) {
    ...
    for (var k = 0; k < N_transfers.length; k++) {
      ...
      if (transfers_exist) {
        for (var m = 0; m < N_stations[transfer_station].lines.length; m++) {
          if (
            !is_in_array(N_stations[transfer_station].lines[m], line_transfers)
          )
    ...
    for (var q = 0; q < line_transfers.length; q++) {
      var html_css_combo =
        N_lines[line_transfers[q]].html + ' ' + N_lines[line_transfers[q]].css;
      if (!is_in_array(html_css_combo, html_css_combos)) {
        station_entry +=
          '<div class="subway-line-mini ' +
          N_lines[line_transfers[q]].css +
          '"><div class="height_fix"></div><div class="content">' +
          N_lines[line_transfers[q]].html +
          '</div></div>';
